...
1. **Block IP**: `mcp_wazuh_wazuh_block_ip`
   ...
   - Linux and Windows supported when targeting one active agent. Windows uses the ORED wrapper, not built-in `netsh.exe`.
...
| Action | Policy | Tool |
|--------|--------|------|
| Block IP | Approval required; Linux and Windows supported for one active agent | `wazuh_block_ip` |
| Firewall drop | Approval required; Linux and Windows supported for one active agent | `wazuh_firewall_drop` |
