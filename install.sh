...
echo -e "${BOLD}${GREEN}════════════════════════════════════════════${RESET}"
echo ""
echo "  MCP Server:  internal Docker service wazuh-mcp-server:3000"
echo "  Health:      $DC exec wazuh-mcp-server curl -sf http://localhost:3000/health"
echo "  Logs:        $DC logs -f"
echo ""
...
echo ""
if [ "$MCP_ONLY" = true ]; then
    echo "  MCP-only mode does not publish port 3000 by default."
    echo "  Connect Docker-networked clients to: http://wazuh-mcp-server:3000/mcp"
    echo "  For a native host Hermes client, add a local-only Compose override first."
    echo ""
else
